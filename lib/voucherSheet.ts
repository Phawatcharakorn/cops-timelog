...
// Draws one "หลักฐานการจ่ายเงิน" sheet listing MULTIPLE students as rows on
// the same table (matching the physical paper form used for department
// payroll — one sheet per department, everyone who worked that month as a
// numbered row), instead of one sheet per student. Used by the "Export
// Excel ทุกคน" bulk export.
export function addCombinedVoucherSheet(
  wb: ExcelJS.Workbook,
  sheetName: string,
  department: string,
  year: number,
  month: number,
  students: { name: string; logs: LogRow[] }[],
) {
  const nDays = daysInMonth(year, month)
  const dayCols = nDays
  const totalCols = 3 + dayCols + 3
  ...
  col(3).width = 10
  for (let i = 0; i < dayCols; i++) col(4 + i).width = 3.2
  col(4 + dayCols).width = 8
  col(5 + dayCols).width = 10
  col(6 + dayCols).width = 14
  ...
  setCell(2, 1, `โครงการเงินสนับสนุนนิสิตทำงานระหว่างเรียน  ประจำปีงบประมาณ ${year + 543}`, { size: 14 })
  merge(3, 1, 3, colCount)
  setCell(3, 1, `แผนก/ฝ่าย: ${department}`, { size: 14 })
  ...
  students.forEach((student, i) => {
    ...
    setCell(r, 1, i + 1)
    setCell(r, 2, student.name, { align: 'left' })
    setCell(r, 3, `${HOURLY_RATE} บ./ชม.`, { size: 12 })
    ...
    for (let d = 1; d <= dayCols; d++) {
      const hrs = hoursByDay[d]
      if (hrs) totalHours += Math.round(hrs * 100) / 100
      setCell(r, 3 + d, hrs ? Math.round(hrs * 100) / 100 : 'x', { size: 12 })
    }
    grandTotalAmount += totalHours * HOURLY_RATE
    setCell(r, 4 + dayCols, { formula: `SUM(${colLetter(4)}${r}:${colLetter(3 + dayCols)}${r})` })
    setCell(r, 5 + dayCols, { formula: `${colLetter(4 + dayCols)}${r}*${HOURLY_RATE}` })
    setCell(r, 6 + dayCols, '')
    for (let c = 1; c <= colCount; c++) border(r, c)
  })
  ...
  const amountStartCol = 3 + Math.floor(dayCols / 2) + 1
  merge(totalRow, amountStartCol, totalRow, 3 + dayCols)
  setCell(totalRow, amountStartCol, `- ${thaiBahtText(grandTotalAmount)} -`, { align: 'left' })
  setCell(totalRow, 4 + dayCols, { formula: `SUM(${colLetter(4 + dayCols)}${firstDataRow}:${colLetter(4 + dayCols)}${lastDataRow})` }, { bold: true })
  setCell(totalRow, 5 + dayCols, { formula: `SUM(${colLetter(5 + dayCols)}${firstDataRow}:${colLetter(5 + dayCols)}${lastDataRow})` }, { bold: true })
