...
export default function DevPage() {
    ...
    try {
      ...
      const res = await fetch('/api/students', {
        ...
        body: JSON.stringify({
          ...
          pin:        pin || null,
          position:   deptToSave === 'Student Assistant' ? (position.trim() || null) : null,
        }),
      })
      if (!res.ok) { const d = await res.json().catch(() => ({})); throw new Error(d.error || res.statusText) }
      showToast('เพิ่มนิสิตเรียบร้อยแล้ว', 'success')
      setAddStudentOpen(false)
      setAddStudentForm({ student_id: '', name: '', nickname: '', department: 'Marketing', faculty: FACULTIES[0], major: '', pin: '', position: '' })
  ...
  const handleAddLog = async () => {
    ...
    try {
      ...
      setAddLogOpen(false)
      setAddLogForm({ date: todayThai(), check_in: '09:00', check_out: '', check_out_date: '', project_name: '', work_summary: '' })
      await fetchSummary(); if (overview.length > 0) void fetchOverview()
    } catch (e) {
    ...
    <div className="min-h-screen bg-gray-50">
      <SdecHeader
        ...
        right={<>
          ...
          <a href="/student" className="text-xs text-white/80 hover:text-white font-medium whitespace-nowrap transition-colors">หน้าบันทึกเวลา</a>
                              ...
                              {log.status === 'approved' ? (
                                <div className="space-y-2">
                                  ...
                                  <div className="flex flex-wrap gap-2 items-center">
                                    <button onClick={() => handleUnapprove(log.id)} disabled={busyLogIds.has(log.id)}
                                      className="text-xs text-gray-400 hover:text-red-500 disabled:opacity-40 transition-colors whitespace-nowrap">
                                      ยกเลิกอนุมัติ
                                    </button>
                                  </div>
                                </div>
                                ...
                                <div className="space-y-1.5">
                                  <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <button onClick={() => handleUndoReject(log.id)} disabled={busyLogIds.has(log.id)}
                                      className="text-xs bg-gray-100 hover:bg-gray-200 disabled:opacity-40 text-gray-600 px-3 py-1.5 rounded-lg font-semibold transition-colors whitespace-nowrap">
                                      ยกเลิกการตีกลับ
                                    </button>
                                  </div>
                                  {log.rejected_reason && (
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  <button onClick={() => handleApprove(log.id)} disabled={busyLogIds.has(log.id)}
                                    className="text-xs bg-green-600 hover:bg-green-700 disabled:opacity-40 text-white px-3 py-1.5 rounded-lg font-semibold transition-colors whitespace-nowrap">
                                    อนุมัติ
                                  </button>
                                  <button onClick={() => { setRejectModal({ id: log.id }); setRejectReason('') }} disabled={busyLogIds.has(log.id)}
                                    className="text-xs bg-red-50 hover:bg-red-100 disabled:opacity-40 text-red-600 px-3 py-1.5 rounded-lg font-semibold transition-colors whitespace-nowrap">
                                    ตีกลับ
                                  </button>
                                </div>
                              )}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 space-y-3">
              <div className="flex flex-wrap items-end justify-between gap-3">
                ...
                <div className="flex flex-wrap items-end gap-2">
                  ...
                  <button onClick={handleGitImport} disabled={importingGit}
                    title="ดึง commit ใหม่จาก GitHub มาสร้าง log รอการอนุมัติ (ประมาณเวลาจากช่วงห่างจริงระหว่าง commit)"
                    className="bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-medium px-4 py-2.5 rounded-lg text-sm flex items-center gap-2 transition-colors whitespace-nowrap">
              ...
              <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-gray-100">
                ...
                <button onClick={handleExportPayroll}
                  title="ใบเบิกจ่ายเงินนิสิตช่วยปฏิบัติงาน (Excel)"
                  className={exportBtnCls}>
                  <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                  </svg>
                ...
                <button onClick={handleExportStudentDetails}
                  title="รายละเอียดเลขที่บัญชีนิสิตช่วยปฏิบัติงาน (Excel)"
                  className={exportBtnCls}>
                  <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                  </svg>
                ...
                <button onClick={handleManualDeleteMonth} disabled={deletingMonth}
                  ...
                  className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg border border-red-200 text-red-700 bg-white hover:bg-red-50 disabled:opacity-50 transition-colors whitespace-nowrap ml-auto">
                  <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
